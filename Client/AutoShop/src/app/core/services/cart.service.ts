...
export class CartService {
  ...
  setCart(cart: Cart) {
    return this.http.post<Cart>(this.baseUrl + 'cart', cart).subscribe({
      next: (_cart) => this.cart.set(_cart),
    });
  }
  addItemToCart(item: CartItem | Product, quantity = 1) {
    const cart = this.cart() ?? this.createCart();
    if (this.isProduct(item)) {
      item = this.mapItemToCartItem(item);
    }
  ...
  deleteCart() {
    this.http
      .delete(this.baseUrl + 'cart?cartId=' + this.cart()?.id)
      .subscribe({
        next: () => {
  ...
  private mapItemToCartItem(item: Product): CartItem {
    return {
      productId: item.id,
